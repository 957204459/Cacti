...
define('RABBITMQCTL_BIN', '/usr/sbin/rabbitmqctl');
...
$rabbitStats = shell_exec(RABBITMQCTL_BIN . ' status');
...
// OTHERS
preg_match('/{sockets_used,([0-9]+)}/', $rabbitStats, $matches);
$results[] = array('sockets_used' => $matches[1]);
preg_match('/{processes.*?{used,([0-9]+)}]}/', $rabbitStats, $matches);
$results[] = array('processes' => $matches[1]);
